...
def Login(inp_id, inp_pw, id, pw) :
    if(inp_id == id & inp_pw == pw):
        {
            True
        }
...
class connectDBMS :
    ...
    def SIGNUP_USERS_INFORMATION(tx):
        a = tx.run("CREATE (u:user{name : $username, id : $user_id, pw : $user_password, nickname:$nickname})",
                   username = "", user_id ="", user_password = "", nickname = "")
        return 'add user information success'

    @staticmethod
    def LOGIN_USERS_INFORMATION(tx):
        c = tx.run("MATCH (u:user{id : $user_id, pw : $user_password})"
                   "RETURN u", user_id ="", user_password = "")
        Login()
